...
#ifdef USE_FAT32
#define sector_t uint32_t
#else
#define sector_t uint16_t
#endif


extern uint8_t fat_buf[512];
extern uint32_t filesize;
extern uint32_t filestart;
...
typedef struct
{
	...
	union
	{
		...
		struct
		{
			uint32_t		bsNumSecPerFAT32;    // 36-39
			uint8_t	    bsExtFlags[2];       // 40,41
			uint8_t     bsSysVer[2];         //42,43
			uint32_t    bsRootDir;           //44-47
			uint8_t     reserved[23];        //48-71
			uint8_t     bsVolLbl32[11];      //71-81
			uint8_t     bsFileSysType32[8];  //82-89
      uint8_t     bsBootstrap32[418];  // Bootstrap
		};
	};
